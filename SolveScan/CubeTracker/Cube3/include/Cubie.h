 ...
 *
 *      When making structures to represent larger-sized cubes with cubies,
 *      you must make position (0, 0, 0) the center/core of the cube for any rotations to work properly.
 *      As an example, a 3x3x3 Cube would have coordinates ranging from (-1, -1, -1) to (1, 1, 1),
 *      symmetrical about (0, 0, 0).
 *
 *      It is recommended to use createCubie() for heap-allocated Cubies.
 *      Cubies allocated that way will be owned by the caller of createCubie().
 *
...
/**
 * @struct MOVESPEC
 *     Represents a specification for a move that can be applied to a Cube. Applying one to a Cubie results in a pure
 *     rotation.
 *
 *     The MOVE member indicates which layer to apply the move to.
 *
 *     The count member indicates the amount of times to consecutively apply the move. On a cube/cubie,
 *     applying a move 4 consecutive times results in a net 0 change. A count of 0 is functionally the same as a
 *     count of 4; both result in no net change to the state of the cube/cubie.
 *
 *     The clockwise member indicates the direction to apply the move when facing the face being rotated.
 *     A value of 1 indicates clockwise, while a value of 0 indicates counterclockwise. Any other values are considered
 *     invalid.
 *
 ...
 *     @endcode
 *     Although applying a single move 3 times results in the same as applying it once in the reverse direction,
 *     those are both treated as separate move specifications. R3 does convey something different to R' even though
 *     they result in the same final cube state.
 *
 **/
...
//The opposite color is defined to be the color on the opposite face on the same axis. COLORS_NULL --> COLORS_NULL
Colors getOppositeColor(Colors COLOR);

/**
 * Applies a rotation onto the source Cubie given the specified MoveSpec.
 * The MOVE member of the MoveSpec* provided corresponds to the following Cubie rotations:
 ...
 *
 *  Although these moves typically result in different layers being turned on a larger cube,
 *  they result in the same rotation on a 1x1x1 Cubie.
 *
 *  Thus, 1 application of MOVE_FRONT clockwise yields the same result as 1 application of MOVE_BACK counterclockwise
 *  as they both are fundamentally 1 clockwise rotation of the Z-axis
 *
 *  @param source The Cubie to apply the rotation to
 *  @param MOVE_SPEC The move specification (MOVE dictates which rotation to do, count is the amount of times to apply
 *  the rotation, and clockwise indicates the direction to apply the rotation)
 *
 *  Only call this function when working on the Cubie level.
 *  If working with a larger sized cube, then make functions to handle moves on that cube for minimal confusion.
 *
 ...
 *      Although the count member of MOVE_SPEC can go up to the integer limit,
 *      the converted string will not use that count in its representation and instead use the net rotations
 ...
 * @return Heap-allocated, null-terminated, char* holding the move formatted by WCA notation.
 ...
 * This is useful for determining if the inspection step of a solve has ended as the inspection step only allows
 * rotations before actual moves.
 * MOVE_NULL counts as a rotation, since it is the same as doing nothing to the Cube
 * (which is the same as a rotation applied 4 times).
 *
...
/**
 *
 * @param source The Cubie being checked for having COLOR
 * @param COLOR  The Color to check for
 * @return Boolean: 1 if COLOR is one of the Colors used on the Cubie source, 0 otherwise (including NULL POINTER)
 */
int isColorOnCubie(const Cubie* source, Colors COLOR);
